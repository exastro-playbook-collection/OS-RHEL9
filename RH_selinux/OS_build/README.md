Ansible Role: OS-RHEL9/RH_selinux/OS_build
=======================================================
# Description
本ロールは、RHEL9に関するSeLinux設定についての情報の設定を行います。
...
OS-RHEL9/RH_selinux/OS_gatheringロールを利用します。
...
| ---- | ----------- | 
| `VAR_RH_selinux` | | 
| `- file` | コピー元ファイルconfigの情報（ファイル名前またはパス付けファイル名前） | 
...
- hosts: all
  gather_facts: true
  roles:
    - role: OS-RHEL9/RH_selinux/OS_build
      VAR_RH_selinux:
      - file: config
        path: /etc/selinux/config
        properties:
...
- hosts: all
  gather_facts: true
  roles:
    - role: OS-RHEL9/RH_selinux/OS_gathering
  strategy: free
~~~
