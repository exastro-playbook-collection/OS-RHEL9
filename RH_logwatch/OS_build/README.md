Ansible Role: OS-RHEL9/RH_logwatch/OS_build
=======================================================
# Description
本ロールは、RHEL9に関するログ監視設定についての情報の設定を行います。
...
OS-RHEL9/RH_logwatch/OS_gatheringロールを利用します。
...
| ---- | ----------- | 
| `VAR_RH_logwatch` | | 
| `- file` | コピー元ファイルlogwatch.confの情報（ファイル名前またはパス付けファイル名前） | 
...
- hosts: all
  gather_facts: true
  roles:
    - role: OS-RHEL9/RH_logwatch/OS_build
      VAR_RH_logwatch:
      - file: logwatch.conf
        path: /etc/logwatch/conf/logwatch.conf
        text:
...
- hosts: all
  gather_facts: true
  roles:
    - role: OS-RHEL9/RH_logwatch/OS_gathering
  strategy: free
~~~
