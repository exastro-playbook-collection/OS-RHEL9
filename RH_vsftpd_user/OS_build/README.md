Ansible Role: OS-RHEL9/RH_vsftpd_user/OS_build
=======================================================
# Description
本ロールは、RHEL9に関するFTPユーザー設定についての情報の設定を行います。
...
OS-RHEL9/RH_vsftpd_user/OS_gatheringロールを利用します。
...
| ---- | ----------- | 
| `VAR_RH_vsftpd_user` | | 
| `- file` | コピー元ファイルftpusersの情報（ファイル名前またはパス付けファイル名前） | 
...
- hosts: all
  gather_facts: true
  roles:
    - role: OS-RHEL9/RH_vsftpd_user/OS_build
      VAR_RH_vsftpd_user:
      - file: ftpusers
        path: /etc/vsftpd/ftpusers
        text:
...
- hosts: all
  gather_facts: true
  roles:
    - role: OS-RHEL9/RH_vsftpd_user/OS_gathering
  strategy: free
~~~
