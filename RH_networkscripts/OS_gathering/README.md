Ansible Role: OS-RHEL9/RH_networkscripts/OS_gathering
=======================================================
# Description
本ロールは、RHEL9に関するインタフェース設定についての情報の取得を行います。
...
| `/etc/NetworkManager/system-connections/*.nmconnection` | /etc/NetworkManager/system-connections/配下の*.nmconnectionのファイル | 
...
| `- file` | コピー元ファイル/etc/sysconfig/network-scripts/配下のifcfg-.*ファイルの情報（ファイル名前またはパス付けファイル名前）、収集不可ので常に''(空値)である | 
