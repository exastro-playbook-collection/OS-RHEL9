Ansible Role: OS-RHEL9/RH_initfunc/OS_build
=======================================================
# Description
本ロールは、RHEL9に関する起動スクリプト関数についての情報の設定を行います。
...
OS-RHEL9/RH_initfunc/OS_gatheringロールを利用します。
...
| ---- | ----------- | 
| `VAR_RH_initfunc` | | 
| `- file` | コピー元ファイルfunctionsの情報（ファイル名前またはパス付けファイル名前） | 
...
- hosts: all
  gather_facts: true
  roles:
    - role: OS-RHEL9/RH_initfunc/OS_build
      VAR_RH_initfunc:
      - file: functions
        path: /etc/init.d/functions
        text:
...
- hosts: all
  gather_facts: true
  roles:
    - role: OS-RHEL9/RH_initfunc/OS_gathering
  strategy: free
~~~
